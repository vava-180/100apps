...
### これから作る Day 30–40（今回の具体案）
- [ ] Day 030 CSV集計・ピボット … 列を選んで合計／件数／平均（`csv`）
- [ ] Day 031 CSV 列の抽出・絞り込み … 必要な列だけ／条件で行を絞る（`csv`）
- [ ] Day 032 フォルダ容量レポート … 重いフォルダを大きい順に一覧（`pathlib`）
- [ ] Day 033 フォルダ台帳CSV出力 … 名前・サイズ・更新日を棚卸しCSVに（`csv` / `pathlib`）
- [ ] Day 034 2ファイル差分比較 … テキスト/CSVの「変わった行」を表示（`difflib`）
- [ ] Day 035 日報／議事録テンプレ自動生成 … 今日の日付入り定型ファイルを作成（`datetime`）
- [ ] Day 036 ZIP一括 圧縮／解凍 … まとめてZIP化／一括展開（`zipfile`）
- [ ] Day 037 差し込みテキスト量産 … 名簿＋ひな形から文面を人数分生成（送信はしない）（`csv`）
- [ ] Day 038 PDFテキスト抽出 … PDFから文字を取り出してテキスト保存（`pypdf` 既導入）
- [ ] Day 039 複数Excel→1ブック集約 … 複数ブックのシートを1ファイルにまとめる（`openpyxl` 既導入）
- [ ] Day 040 ファイル一括 文字数／行数統計 … フォルダ内テキストの文字数・行数を集計（標準ライブラリ）
